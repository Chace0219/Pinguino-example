...
#define USBEnableInterrupts() {IEC1bits.USBIE = 1;IPC11CLR=0x0000FF00;IPC11SET=0x00001000; unsigned int val;asm volatile("mfc0   %0,$13" : "=r"(val)); val |= 0x00800000; asm volatile("mtc0   %0,$13" : "+r"(val)); INTCONSET = _INTCON_MVEC_MASK; unsigned int status = 0; asm volatile("ei    %0" : "=r"(status)); }
// set the CP0 status IE bit high to turn on interrupts
//_INTEnableInterrupts();
